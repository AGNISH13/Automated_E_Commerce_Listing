...
class Pipeline:
    def __init__(self):
        ...
        self.transcript = ""
        self.transcript_obj = transcripts()
        
        self.keyframe_obj = keyframes()
        self.keyframe_class_obj = keyframe_classes()

        self.final_class_obj = final_class() 
        self.description = ""
    ...
    def create_video_obj(self):
        self.video_id = self.videoobj.create(video_link=self.video_path)
        # set stepper db
        self.step = 1
    ...
    def run_steps(self): # update with stepper
        ...
        if self.step == 1:
            ...
            self.transcipt_id = self.transcript_obj.create(video_id=self.video_id, transcript=self.transcript)
            ...
            print("Extracted")
            self.keyframes_id = self.keyframe_obj.create(video_id=self.video_id,keyframes=keyframe_bins)
            print("sent to db")
            # self.step +=1 #!Filtered keyframes error

            self.katna_keyframes = keyframe_bins[0:2]
            self.flag_katna = True
            self.end()
        ...
        # elif self.step == 3:
            ...
            # for keyframe_collection in self.keyframe_obj.get_all_keyframes(video_id=self.video_id)['response']:
            #     keyframe_r.append(keyframe_collection)
            # # print(keyframe_r)
            # filtered_keyframes = filter_images(keyframe_r)
            # self.keyframe_obj.update_keyframes(self.video_id, filtered_keyframes)
            ...
            filtered_keyframes=[]
            for keyframe_collection in self.keyframe_obj.get_all_keyframes(video_id=self.video_id)['response']:
                filtered_keyframes.append(keyframe_collection)
            ...
            for classid in class_ids:
                self.keyframe_class_obj.create(video_id=classid['video_id'],keyframe_id=classid['keyframe_id'],classes=classid['class'])
            # print(class_ids[-1])
            ...
            class_ids = self.keyframe_class_obj.get_classes(video_id=self.video_id)['response']
            confirmed_ids = identify_product(self.transcript,class_ids)  # Example user input
            self.final_class_obj.create(self.video_id,classes=confirmed_ids)
            ...
            confirmed_ids = self.final_class_obj.get_final_classes(video_id=self.video_id)['response']
            ...
            print("Opore")
            product_description = generate_product_description(self.transcript)
            print("Niche")
